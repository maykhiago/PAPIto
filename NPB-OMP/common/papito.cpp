...
void papito_start()
{
  ...
  for (int i = 0; i < papiEvents.size(); i++)
  {
    ...
    //cout << &num_event_set[num_thread] << " - " << num_event_set[num_thread] << " - " << num_thread << endl;
		// Create the Event Set
		if (PAPI_create_eventset(&num_event_set[num_thread]) != PAPI_OK) papito_handle_error(1);

    //cout << num_event_set[num_thread] << " - " << papiEvents.size() << endl;
